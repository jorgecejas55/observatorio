// Servicio de autenticación para Museo Arqueológico Adán Quiroga
// Maneja estado de sesión y comunicación con Google Apps Script

const MUSEO_AUTH_URL = process.env.NEXT_PUBLIC_MUSEO_ADAN_QUIROGA_SCRIPT_URL || ''
const STORAGE_KEY = 'museo_adan_quiroga_session'
...
const MuseoAdanQuirogaAuthService = {
  /**
   * Helper para peticiones GET al Apps Script
   */
  async _requestGet(params: Record<string, string>): Promise<any> {
    if (!MUSEO_AUTH_URL) {
      console.error('⚠️ NEXT_PUBLIC_MUSEO_ADAN_QUIROGA_SCRIPT_URL no está definida en .env.local')
    ...
    try {
      ...
      console.error('Error en request Apps Script (Adán Quiroga):', error)
      throw error
    }
  },
  ...
  async login(email: string, password: string): Promise<LoginResponse> {
    try {
      ...
      console.error('❌ Error en login (Adán Quiroga):', error)
...
export default MuseoAdanQuirogaAuthService
