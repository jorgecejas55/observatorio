...
function VisitasOcasionalesContent() {
  ...
  const handleSave = useCallback(async (formData: any) => {
    try {
      ...
      if (isEditing) {
        const res = await fetch(`/api/ocio/ingresos/museo-casa-caravati/ocasionales/${editando.id}`, {
          method: 'PUT',
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/ocio/ingresos/museo-casa-caravati/ocasionales/${id}`, {
        method: 'DELETE',
      })
  ...
  const handleDeleteMultiple = useCallback(async (ids: string[]) => {
    ...
    try {
      const promesas = ids.map(id =>
        fetch(`/api/ocio/ingresos/museo-casa-caravati/ocasionales/${id}`, {
          method: 'DELETE',
        }).then(res => res.json())
      )
  ...
  const exportarCSV = useCallback(() => {
    ...
    link.setAttribute('download', `visitas-ocasionales-casa-caravati-${fecha}.csv`)
  ...
  const exportarXLSX = useCallback(() => {
    ...
    // Descargar
    const fecha = new Date().toISOString().split('T')[0]
    XLSX.writeFile(wb, `visitas-ocasionales-casa-caravati-${fecha}.xlsx`)
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link
            href="/ocio/ingresos/museo-casa-caravati/ocasionales/carga-masiva"
