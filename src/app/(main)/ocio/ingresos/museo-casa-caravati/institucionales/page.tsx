...
function VisitasInstitucionalesContent() {
  ...
  const cargarVisitas = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/ocio/ingresos/museo-casa-caravati/institucionales')
  ...
  const handleSave = useCallback(async (formData: any) => {
    try {
      ...
      if (isEditing) {
        const res = await fetch(`/api/ocio/ingresos/museo-casa-caravati/institucionales/${editando.id}`, {
          method: 'PUT',
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/ocio/ingresos/museo-casa-caravati/institucionales/${id}`, {
        method: 'DELETE',
      })
  ...
  const handleDeleteMultiple = useCallback(async (ids: string[]) => {
    ...
    try {
      const promesas = ids.map(id =>
        fetch(`/api/ocio/ingresos/museo-casa-caravati/institucionales/${id}`, {
          method: 'DELETE',
        }).then(res => res.json())
      )
  ...
  const exportarCSV = useCallback(() => {
    ...
    link.setAttribute('download', `visitas-institucionales-casa-caravati-${fecha}.csv`)
  ...
  const exportarXLSX = useCallback(() => {
    ...
    // Descargar
    const fecha = new Date().toISOString().split('T')[0]
    XLSX.writeFile(wb, `visitas-institucionales-casa-caravati-${fecha}.xlsx`)
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-text-secondary text-sm">
            Museo de la Ciudad - Casa Caravati · {loading ? 'Cargando...' : `${visitasFiltradas.length} de ${visitas.length} visita${visitas.length !== 1 ? 's' : ''}`}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link
            href="/ocio/ingresos/museo-casa-caravati/institucionales/carga-masiva"
