...
function VisitasInstitucionalesContent() {
  ...
  const cargarVisitas = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/ocio/ingresos/museo-adan-quiroga/institucionales')
  ...
  const handleSave = useCallback(async (formData: any) => {
    try {
      ...
      if (isEditing) {
        const res = await fetch(`/api/ocio/ingresos/museo-adan-quiroga/institucionales/${editando.id}`, {
          method: 'PUT',
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/ocio/ingresos/museo-adan-quiroga/institucionales/${id}`, {
        method: 'DELETE',
      })
  ...
  const exportarCSV = useCallback(() => {
    ...
    link.setAttribute('download', `visitas-institucionales-adan-quiroga-${fecha}.csv`)
  ...
  const exportarXLSX = useCallback(() => {
    ...
    // Descargar
    const fecha = new Date().toISOString().split('T')[0]
    XLSX.writeFile(wb, `visitas-institucionales-adan-quiroga-${fecha}.xlsx`)
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link
            href="/ocio/ingresos/museo-adan-quiroga/institucionales/carga-masiva"
