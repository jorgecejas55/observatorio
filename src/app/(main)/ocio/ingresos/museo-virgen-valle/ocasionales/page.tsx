...
function VisitasOcasionalesContent() {
  ...
  const handleSave = useCallback(async (formData: any) => {
    try {
      ...
      if (isEditing) {
        const res = await fetch(`/api/ocio/ingresos/museo-virgen-valle/ocasionales/${editando.id}`, {
          method: 'PUT',
  ...
  const handleDelete = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/ocio/ingresos/museo-virgen-valle/ocasionales/${id}`, {
        method: 'DELETE',
      })
  ...
  const exportarCSV = useCallback(() => {
    ...
    link.setAttribute('download', `visitas-ocasionales-virgen-valle-${fecha}.csv`)
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Link
            href="/ocio/ingresos/museo-virgen-valle/ocasionales/carga-masiva"
