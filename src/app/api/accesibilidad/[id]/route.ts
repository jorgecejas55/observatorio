...
const FIELDS_DETALLE = [
  ...
  'fotos_acceso_exterior.directus_files_id',
  'fotos_circulacion.directus_files_id',
  'fotos_servicios.directus_files_id',
  'fotos_visual.directus_files_id',
  'fotos_auditiva.directus_files_id',
  'fotos_cognitiva.directus_files_id',
].join(',')
...
export async function GET(
  ...
  try {
    ...
    const url = new URL(`${DIRECTUS_URL}/items/acc_atractivos/${id}`)
    url.searchParams.set('fields', FIELDS_DETALLE)
    ...
    if (!res.ok) {
      ...
      console.error('[accesibilidad/id] Directus error:', res.status, err)
    ...
    console.error('[accesibilidad/id]', err)
