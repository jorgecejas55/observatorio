/**
 * GOOGLE APPS SCRIPT - MUSEO DE LA CIUDAD - CASA CARAVATI
 ...
 * 6. Copiar URL y agregar a .env.local como MUSEO_CASA_CARAVATI_SCRIPT_URL
 */

// ID del Google Sheet (configuración)
const SHEET_ID = '1kBIMwf7I5KVDxBHz9IuiyW0J2KZY6W4P4KcMlt7Pshs';
...
function setup() {
  ...
  Logger.log('=== SETUP MUSEO DE LA CIUDAD - CASA CARAVATI ===');
  ...
  if (wsUsers.getLastRow() === 0) {
    ...
    Logger.log('✅ Hoja "' + SHEETS.USUARIOS + '" creada con usuario admin@casa-caravati.com / 123456');
  } else {
...
function testGetInstitucionales() {
  const data = getSheetData(SHEETS.INSTITUCIONALES);
  Logger.log('Total visitas institucionales: ' + data.length);
  if (data.length > 0) {
    Logger.log('Primera visita:');
    Logger.log(JSON.stringify(data[0], null, 2));
  } else {
    Logger.log('No hay visitas institucionales registradas');
  }
}

function testLogin() {
  const user = verifyUser('admin@casa-caravati.com', '123456');
...
function testLoginDetallado() {
  ...
  const email = 'admin@casa-caravati.com';
